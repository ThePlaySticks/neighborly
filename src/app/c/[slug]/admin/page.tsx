...
import { ShieldCheck, Users, Megaphone, Settings, CheckCircle2, UserCheck, AlertTriangle } from 'lucide-react'
...
export default async function CommunityAdminPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!currentUser) {
    redirect(`/login?redirect=/c/${slug}/admin`)
  }

  const membership = dal.getMembership(currentUser.id, community.id)
  if (!membership || (membership.role !== 'COMMUNITY_ADMIN' && membership.role !== 'SUPER_ADMIN')) {
    return (
        ...
        <main className="flex-1 flex items-center justify-center p-4">
          <Card className="p-8 max-w-md text-center space-y-4 border-destructive/20 bg-card">
            ...
            <p className="text-xs text-muted-foreground">
              You do not have administrator rights for {community.name}. Only verified community administrators can access this portal.
            </p>
            <Link href={`/c/${slug}`}>
              <Button size="sm" variant="outline">Back to Estate</Button>
            </Link>
          </Card>
        </main>
        <Footer />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-black text-foreground">{community.name} Admin</h1>
                <Badge variant="outline" className="text-xs bg-primary/10 text-primary border-primary/20">
                  Estate Management
                </Badge>
              </div>
              <p className="text-xs text-muted-foreground">
                Logged in as <strong>{currentUser.fullName}</strong> ({membership.role})
              </p>
            </div>
          </div>
